...
function generateRandomSources(width, height, numberOfSources, maxAccumulation) {
    const totalCells = width * height;
    const sources = new Float64Array(totalCells).fill(0);
    let remainingSources = numberOfSources;
...
const sourcesTestCases = [
    { description: "Few sources and sinks", numSources: 3, maxAccumulation: 3 },
    { description: "Moderate sources and sinks", numSources: 10, maxAccumulation: 3},
    { description: "Many sources and sinks", numSources: 20, maxAccumulation: 3 },
];
...
describe("Analytic vs Numerical Steady-State Solution", () => {
    ...
    const WIDTH = 100;
    const HEIGHT = 60;
    const DECAY_RATE = 0.1; // per second //this will never be zero
    const deltaT = 0.2; // seconds
    const maxMode = 100;

    test.each(sourcesTestCases)(
        "$description",
        ({  numSources, maxAccumulation }) => {

            // Arrange
            const sources = generateRandomSources(WIDTH, HEIGHT, numSources, maxAccumulation);
